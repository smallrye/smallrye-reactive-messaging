        ...
        if (extension != null) {
            if (extension.kafka != null) {
                ...
                if (parameterContext.getParameter().getType().equals(KafkaProxy.class)) {
                    StrimziKafkaContainer strimzi = (StrimziKafkaContainer) extension.kafka;
                    Proxy proxyForNode = extension.cluster.getProxyForNode(strimzi.getNodeId());
                    return new KafkaProxy(proxyForNode, extension.kafka.getContainerIpAddress(),
                            extension.kafka.getMappedPort(9092), 0);
                }
            }
        }
        return null;
