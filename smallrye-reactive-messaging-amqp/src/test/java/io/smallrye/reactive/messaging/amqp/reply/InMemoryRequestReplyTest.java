...
public class InMemoryRequestReplyTest {
    ...
    public void initWeld() {
        ...
        weld.addBeanClass(AmqpRequestReplyFactory.class);
    ...
    @Test
    void testAmqpRequestReplyWithInMemoryConnector() {
        ...
        AmqpRequestReply<String, String> rr = application.requestReply();
        assertThat(rr).isInstanceOf(NoOpAmqpRequestReplyImpl.class);

        ((NoOpAmqpRequestReplyImpl<String, String>) rr)
                .setReplyFunction(req -> "amqp-reply-" + req);

        String reply = rr.request("hello").await().indefinitely();
        assertThat(reply).isEqualTo("amqp-reply-hello");
    ...
    @Test
    void testAmqpRequestReplyPendingRepliesAndComplete() {
        MapBasedConfig config = new MapBasedConfig()
                .with("mp.messaging.outgoing.rr-out.connector", InMemoryConnector.CONNECTOR);
