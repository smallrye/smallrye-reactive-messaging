...
Otherwise, you can use _raw_ `Message` and use _metadata_.
Note that using `HttpMessage` is producing a _raw_ `Message` using metadata.
...
The following table list the configurable metadata:
...
|`http.outgoing-query-parameters`
|`Map<String, ?>`
