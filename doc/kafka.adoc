...
When receiving a message from Kafka, you get an instance of `Message` with the following metadata:
...
This `KafkaMessage` provides method to access the value stored in the metadata listed above.
...
When sending a message to Kafka, you can either use a `KafkaMessage` or a _raw_ `Message` with the following metadata:
