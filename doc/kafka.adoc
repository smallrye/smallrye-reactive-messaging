...
mp.messaging.provider.incoming.kafka-heartbeat.type=io.smallrye.reactive.messaging.kafka.Kafka
mp.messaging.provider.incoming.kafka-heartbeat.topic=heartbeat
mp.messaging.provider.incoming.kafka-heartbeat.bootstrap.servers=localhost:9092
mp.messaging.provider.incoming.kafka-heartbeat.enable.auto.commit=false
mp.messaging.provider.incoming.kafka-heartbeat.group.id=demo
mp.messaging.provider.incoming.kafka-heartbeat.key.deserializer=org.apache.kafka.common.serialization.StringDeserializer
mp.messaging.provider.incoming.kafka-heartbeat.value.deserializer=io.vertx.kafka.client.serialization.JsonObjectDeserializer
# [/Source - heartbeat]
----

The `type` attribute is required to indicate you are configuring the Kafka transport. Supported attributes are listed
...
| topic | true |  | The topic to consume
| bootstrap.servers | true | | the comma separated list of servers (host:port)
| key.deserializer | true | | the codec used to deserialize the key
| value.deserializer | true | | the codec used to deserialize the value
| group.id | true | | the group id
...
| topic | false |  | The topic to consume, if not set it must be set in the `message`
| bootstrap.servers | true | | the comma separated list of servers (host:port)
| key.serializer | true | | the codec used to serialize the key
| value.serializer | true | | the codec used to serialize the value
| key | false | | the key to be used, can be overridden by the message
| partition | false | 0 | the partition
| timestamp | false | `null` | the timestamp
|===
