...
public class IncomingMetadataInjectionProcessorReturningPublisherBuilderTest extends WeldTestBaseWithoutTails {
    ...
    void testMetadataInjectingInProcessorIngestingPayload() {
        ...
        initialize();
        IncomingMetadataInjectionProcessorReturningPublisherBuilderTest.Sink sink = container
                .select(IncomingMetadataInjectionProcessorReturningPublisherBuilderTest.Sink.class).get();
    ...
    public static class ProcessorIngestingPayload {
        @Incoming("source")
        @Outgoing("sink")
        public PublisherBuilder<String> process(String p, SimplePropagationTest.MsgMetadata metadata) {
            assertThat(p).isNotNull();
            assertThat(metadata).isNotNull();
            assertThat(metadata.getMessage()).isEqualTo("foo");
            return ReactiveStreams.of(p);
        }
    }
    ...
    public static class ProcessorIngestingPayloadWithOptional {
        @Incoming("source")
        @Outgoing("sink")
        public PublisherBuilder<String> process(String p, Optional<SimplePropagationTest.MsgMetadata> metadata) {
            assertThat(p).isNotNull();
