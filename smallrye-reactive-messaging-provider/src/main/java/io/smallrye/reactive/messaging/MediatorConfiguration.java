...
public class MediatorConfiguration {
        ...
        if (ClassUtils.isAssignable(returnType, Processor.class)
            ...
            if (ClassUtils.isAssignable(returnType, CompletionStage.class)) {
                ...
                Type type = getParameterFromReturnType(method, 0)
                        .orElseThrow(
                                () -> getIncomingAndOutgoingError("Expected a type parameter in the return CompletionStage"));
                production = TypeUtils.isAssignable(type, Message.class) ? Production.COMPLETION_STAGE_OF_MESSAGE
                        : Production.COMPLETION_STAGE_OF_PAYLOAD;
                consumption = ClassUtils.isAssignable(param, Message.class) ? Consumption.MESSAGE : Consumption.PAYLOAD;
            } else {
                // Case 9 or 10
                production = ClassUtils.isAssignable(returnType, Message.class) ? Production.INDIVIDUAL_MESSAGE
                        : Production.INDIVIDUAL_PAYLOAD;
                consumption = ClassUtils.isAssignable(param, Message.class) ? Consumption.MESSAGE : Consumption.PAYLOAD;
            }
        }
    ...
    private void validateMethodReturningAProcessor() {
        ...
        consumption = TypeUtils.isAssignable(type1, Message.class) ? Consumption.STREAM_OF_MESSAGE
                : Consumption.STREAM_OF_PAYLOAD;
    ...
    private void validatePublisher(Outgoing outgoing) {
        ...
        if (ClassUtils.isAssignable(returnType, CompletionStage.class)) {
            ...
            production = TypeUtils.isAssignable(t, Message.class) ? Production.COMPLETION_STAGE_OF_MESSAGE
                    : Production.COMPLETION_STAGE_OF_PAYLOAD;
            return;
        }
    ...
    private void validateSubscriber(Incoming incoming) {
        ...
        if (ClassUtils.isAssignable(returnType, Subscriber.class)
            ...
            consumption = TypeUtils.isAssignable(p, Message.class) ? Consumption.STREAM_OF_MESSAGE
                    : Consumption.STREAM_OF_PAYLOAD;
            return;
        }
