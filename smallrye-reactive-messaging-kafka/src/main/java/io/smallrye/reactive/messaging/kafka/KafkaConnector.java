...
@ConnectorAttribute(name = "ordered", type = "string", direction = Direction.INCOMING, description = "Configures ordering guarantees for concurrent processing. Possible values: `key` for per-key sequential processing, or `partition` for per-partition sequential processing. Requires `@Blocking(ordered = false)` for concurrency. Works with any commit strategy.")
@ConnectorAttribute(name = "ordered.max-concurrency", type = "int", direction = Direction.INCOMING, description = "Maximum number of groups (keys or partitions) that can be processed concurrently. Defaults to `max.poll.records`.")
@ConnectorAttribute(name = "throttled.ordered", type = "string", direction = Direction.INCOMING, description = "While using the `throttled` commit-strategy, configures ordering guarantees for concurrent processing. Possible values: `key` for per-key sequential processing within partitions, or `partition` for per-partition sequential processing. Requires `@Blocking(ordered = false)` for concurrency. Deprecated: Use `ordered` instead.", deprecated = true)
@ConnectorAttribute(name = "throttled.ordered.max-concurrency", type = "int", direction = Direction.INCOMING, description = "While using the `throttled` commit-strategy with ordered processing, specifies the maximum number of groups (keys or partitions) that can be processed concurrently. This should typically match your worker pool's `max-concurrency` setting. Defaults to `max.poll.records`. Deprecated: Use `ordered.max-concurrency` instead.", deprecated = true)
