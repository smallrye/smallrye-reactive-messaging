...
class KeyValueFromRecordExtractorTest extends KafkaCompanionTestBase {
    ...
    public static class AppWithDefaultButDoBeUSedWithCustom {
        ...
        public Multi<String> reshape(KeyedMulti<String, String> keyed) {
            ...
            return keyed
                    ...
                    .map(s -> keyed.key() + "-" + s.get()); // The extractor should to the uppercase
        }
    ...
    public static class AppWithKeyedUsingDefault {
        ...
        @Incoming("in")
        @Outgoing("out")
        public Multi<String> reshape(@Keyed(KeyValueFromKafkaRecordExtractor.class) KeyedMulti<String, String> keyed) {
            assertThat(keyed.key()).isNotNull().isNotBlank();
            return keyed
                    .select().distinct()
                    .onItem().scan(AtomicInteger::new, (count, s) -> {
