...
| `io.vertx.core.json.JsonObject` or `io.vertx.core.json.JsonArray` | AMQP Data using a binary content. The `content-type` is set to `application/json`
...
When sending `Messages`, you can add an instance of `io.smallrye.reactive.messaging.amqp.OutgoingAmqpMetadata` to influence how the message is going to sent to AMQP.
