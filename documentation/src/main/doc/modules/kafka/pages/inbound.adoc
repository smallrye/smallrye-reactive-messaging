...
The connector pauses the underlying Kafka client if there is no downstream consumption.
Periodically, it checks if the downstream demand reaches 0 for a period of time, and if so, pause the polling.

The polling is resumed as soon as the downstream emits another request.

The period of inactivity can be configured using the `pause-after-inactivity` attribute, and is set to 5 seconds by default.
Setting this atribute to 0 disables the mechanism, meaning the underlying Kafka client is not paused.
