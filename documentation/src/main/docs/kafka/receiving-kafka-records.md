...
-   `throttled` keeps track of received messages and commit to the next
    ...
    guarantees using the `throttled.processing-order` configuration. See
...
You can configure processing order guarantees using the `throttled.processing-order` attribute
...
- **`ordered_by_key`**: Messages with the same key from the same topic-partition are
...
- **`ordered_by_partition`**: Messages from the same topic-partition are processed
...
When using ordered processing modes, configure the `throttled.processing-order`
...
mp.messaging.incoming.orders.throttled.processing-order=ordered_by_key
...
The `max-queue-size-factor` (default: 2) attribute controls the buffer size, calculated as
